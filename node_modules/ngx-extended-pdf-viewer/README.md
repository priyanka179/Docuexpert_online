...
## About the version 11.0.0 (currently under development)

Version 11 requires you to use Ivy. If you're stuck with an older Angular version or if you can't update to Ivy, use version 10. The latest version supporting the old View Engine is version 11.0.0-alpha.7 - but that's an unfinished alpha version, so use it at your own risk. That said, I believe that alpha version has a decent quality, so it's not to big a risk.

Version 11.0.0 also updates to pdf.js 2.11 (default) and 2.12 (bleeding-edge) branch. Plus, I've continued to work on the performance, because some developers ask me to get rid the of current limit of several thousand pages. If you're dealing with such a huge document, you'll love the improved single-page mode of the "bleeding-edge" branch because it mostly eliminates the performance penalty of huge documents.
## Showcase and manual
...
*Hint*: If you're missing an attribute, also have a look at the [default options page of the showcase](https://pdfviewer.net/extended-pdf-viewer/default-options)</a>. That's a collection of less used attributes.
...
| [scrollMode]                 |        0     | Determines whether the pages scroll vertically (`[scrollMode]="0"`) or horizontally like a script role (`[scrollMode]="1"`). The third option is the block mode (`[scrollMode]="2"`), which also scrolls vertically, but arranges as many pages as the screen width allows side-by-side.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
